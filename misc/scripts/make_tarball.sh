...
if [ ! -e "version.py" ]; then
  echo "This script should be ran from the root folder of the BlackForest repository."
  exit 1
fi
...
while getopts "h?sv:g:" opt; do
  ...
  h|\?)
    ...
    echo
    echo "  -s script friendly file name (blackforest.tar.gz)"
    echo "  -v blackforest version for file name (e.g. 4.0-stable)"
...
if [ ! -z "$script_friendly_name" ]; then
  ...
  if [ ! -z "$godot_version" ]; then
    NAME=blackforest-$godot_version
  else
    NAME=blackforest-$HEAD
  fi
fi

CURDIR=$(pwd)
TMPDIR=$(mktemp -d -t blackforest-XXXXXX)
