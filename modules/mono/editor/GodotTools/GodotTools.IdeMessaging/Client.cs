    ...
    public sealed class Client : IDisposable
    {
        ...
        public Client(string identity, string godotProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            ...
            if (!Directory.Exists(projectMetadataDir))
            {
                ...
                string nonHiddenProjectMetadataDir = Path.Combine(godotProjectDir, "blackforest", "mono", "metadata");
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                logger.LogInfo("Connecting to BlackForest Ide Server");

                await tcpClient.ConnectAsync(IPAddress.Loopback, godotIdeMetadata.Port);

                logger.LogInfo("Connection open with BlackForest Ide Server");
            ...
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                    logger.LogError("The connection to the BlackForest Ide Server was refused");
                else
                    throw;
            }
        }
        ...
        public async void Start()
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (!File.Exists(MetaFilePath))
                {
                    logger.LogInfo("There is no BlackForest Ide Server running");
                    return;
                }
                ...
                {
                    logger.LogError("Failed to read BlackForest Ide metadata file");
                }
            }
        }
            ...
            where TResponse : Response, new()
        {
            if (!IsConnected)
            {
                logger.LogError("Cannot write request. Not connected to the BlackForest Ide Server.");
                return null;
            }
