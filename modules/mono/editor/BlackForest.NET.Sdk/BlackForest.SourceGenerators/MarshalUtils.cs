...
namespace BlackForest.SourceGenerators
{
    internal static class MarshalUtils
    {
                ...
                default:
                {
                    ...
                    if (typeKind == TypeKind.Struct)
                    {
                        if (type.ContainingAssembly?.Name == "GodotSharp" &&
                            type.ContainingNamespace?.Name == "BlackForest")
                        {
                            return type switch
                            {
                    ...
                    else if (typeKind == TypeKind.Array)
                    {
                        ...
                        if (elementType.ContainingAssembly?.Name == "GodotSharp" &&
                            elementType.ContainingNamespace?.Name == "BlackForest")
                        {
                            switch (elementType)
                            {
                                case { Name: "Vector2" }:
                    ...
                    else
                    {
                        ...
                        if (type.ContainingAssembly?.Name == "GodotSharp")
                        {
                            switch (type.ContainingNamespace?.Name)
                            {
                                ...
                                case "Collections"
                                    when type.ContainingNamespace?.FullQualifiedNameOmitGlobal() == "BlackForest.Collections":
                                    return type switch
                                    {
                                        { Name: "Dictionary" } =>
        ...
        private const string VariantUtils = "global::BlackForest.NativeInterop.VariantUtils";
            ...
            return marshalType switch
            {
                ...
                MarshalType.GodotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".ConvertToSystemArrayOfGodotObject<",
                        ...
                        inputExpr, ")"),
                // We need a special case for generic BlackForest collections and GodotObjectOrDerived[], because VariantUtils.ConvertTo<T> is slower
                MarshalType.GodotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                ...
                // We need a special case for generic BlackForest collections and GodotObjectOrDerived[], because VariantUtils.CreateFrom<T> is slower
                MarshalType.GodotGenericDictionary =>
            ...
            return marshalType switch
            {
                ...
                MarshalType.GodotObjectOrDerivedArray =>
                    source.Append(inputExpr, ".AsGodotObjectArray<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">()"),
                // We need a special case for generic BlackForest collections and GodotObjectOrDerived[], because Variant.As<T> is slower
                MarshalType.GodotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                ...
                MarshalType.GodotObjectOrDerivedArray =>
                    source.Append("global::BlackForest.Variant.CreateFrom(", inputExpr, ")"),
                // We need a special case for generic BlackForest collections, because Variant.From<T> is slower
                MarshalType.GodotGenericDictionary or MarshalType.GodotGenericArray =>
                    source.Append("global::BlackForest.Variant.CreateFrom(", inputExpr, ")"),
                _ => source.Append("global::BlackForest.Variant.From<",
                    typeSymbol.FullQualifiedNameIncludeGlobal(), ">(", inputExpr, ")")
            };
        }
    }
