using BlackForest;
using System;
using static BlackForest.Mathf;
...
public partial class ExportedProperties(string primaryCtorParameter) : GodotObject
{
    ...
    [Export] private BlackForest.Texture PropertyGodotResourceTexture { get; set; }
    [Export] private BlackForest.Texture PropertyGodotResourceTextureWithInitializer { get; set; } = new() { ResourceName = "" };
    ...
    [Export]
    private BlackForest.Collections.Dictionary PropertyGodotDictionary { get; set; } = new() { { "foo", 10 }, { Vector2.Up, Colors.Chocolate } };

    [Export]
    private BlackForest.Collections.Array PropertyGodotArray { get; set; } = new() { "foo", 10, Vector2.Up, Colors.Chocolate };

    [Export]
    private BlackForest.Collections.Dictionary<string, bool> PropertyGodotGenericDictionary { get; set; } = new() { { "foo", true }, { "bar", false } };

    [Export]
    private BlackForest.Collections.Array<int> PropertyGodotGenericArray { get; set; } = new() { 0, 1, 2, 3, 4, 5, 6 };
