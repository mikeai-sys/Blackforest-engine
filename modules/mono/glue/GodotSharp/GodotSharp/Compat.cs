// This file contains methods that existed in a previous version of BlackForest in ClassDB but were removed
...
{
    /// <inheritdoc cref="SegmentIntersectsConvex(Vector3, Vector3, BlackForest.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public static Vector3[] SegmentIntersectsConvex(Vector3 from, Vector3 to, BlackForest.Collections.Array planes)
    {
        return SegmentIntersectsConvex(from, to, new BlackForest.Collections.Array<Plane>(planes));
    }
}
...
{
    /// <inheritdoc cref="AddSurface(Mesh.PrimitiveType, BlackForest.Collections.Array, BlackForest.Collections.Array{BlackForest.Collections.Array}, BlackForest.Collections.Dictionary, Material, string, ulong)"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddSurface(Mesh.PrimitiveType primitive, BlackForest.Collections.Array arrays, BlackForest.Collections.Array<BlackForest.Collections.Array> blendShapes, BlackForest.Collections.Dictionary lods, Material material, string name, uint flags)
    {
...
{
    /// <inheritdoc cref="DrawListBegin(Rid, InitialAction, FinalAction, InitialAction, FinalAction, Color[], float, uint, Nullable{Rect2}, BlackForest.Collections.Array{Rid})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public long DrawListBegin(Rid framebuffer, InitialAction initialColorAction, FinalAction finalColorAction, InitialAction initialDepthAction, FinalAction finalDepthAction, Color[] clearColorValues, float clearDepth, uint clearStencil, Nullable<Rect2> region, BlackForest.Collections.Array storageTextures)
    {
        return DrawListBegin(framebuffer, initialColorAction, finalColorAction, initialDepthAction, finalDepthAction, clearColorValues, clearDepth, clearStencil, region, new BlackForest.Collections.Array<Rid>(storageTextures));
    }
}
...
{
    /// <inheritdoc cref="AddTriangleFan(Vector3[], Vector2[], Color[], Vector2[], Vector3[], BlackForest.Collections.Array{Plane})"/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public void AddTriangleFan(Vector3[] vertices, Vector2[] uvs, Color[] colors, Vector2[] uv2S, Vector3[] normals, BlackForest.Collections.Array tangents)
    {
        AddTriangleFan(vertices, uvs, colors, uv2S, normals, new BlackForest.Collections.Array<Plane>(tangents));
    }
