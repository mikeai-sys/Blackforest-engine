...
def generate_bundle(target, source, env):
    ...
    if env.editor_build:
        # Editor bundle.
        prefix = "blackforest." + env["platform"] + "." + env["target"]
        if env.dev_build:
            prefix += ".dev"
        ...
        if target_bin != "":
            shutil.copy(target_bin, app_dir + "/Contents/MacOS/BlackForest")
            ...
            with open(app_dir + "/Contents/Info.plist", "wt", encoding="utf-8", newline="\n") as fout:
                for line in fin:
                    ...
                    if version["build"] != "official" and version["build"] != "steam":
                        line = line.replace("org.blackforestengine.blackforest", "org.blackforestengine.blackforest." + version["build"])
                    fout.write(line)
        ...
        # Template bundle.
        app_prefix = "blackforest." + env["platform"]
        rel_prefix = "blackforest." + env["platform"] + "." + "template_release"
        dbg_prefix = "blackforest." + env["platform"] + "." + "template_debug"
        if env.dev_build:
            app_prefix += ".dev"
            rel_prefix += ".dev"
            dbg_prefix += ".dev"
